...
; binary at that same path (gui/binary_swap.py), so shortcuts never break and
...
[Setup]
; AppId names the uninstall registry key (DeepReefMap_is1); bootstrap.py
; depends on it — keep in sync.
AppId=DeepReefMap
