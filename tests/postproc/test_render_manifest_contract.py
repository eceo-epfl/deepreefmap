...
def test_render_offline_video_uses_manifest(tmp_path) -> None:
...
def test_render_offline_video_4panel_layout_and_cumulative_ortho(tmp_path) -> None:
    h, w = 16, 24
    n_frames = 3
...
def test_render_offline_video_reads_legacy_npy_labels(tmp_path) -> None:
    """Manifests archived before the PNG migration record `.npy` label paths.

    cv2.imread yields None for a `.npy`, which used to leave the segmentation
    quadrant empty. The black input frame isolates its contribution to the blend.
    """
    h, w = 16, 24
    frames = tmp_path / "frames"
    labels = tmp_path / "labels"
    frames.mkdir()
    labels.mkdir()
    _write_classes_yaml(tmp_path / "classes.yaml")

    cv2.imwrite(str(frames / "00000000.png"), np.zeros((h, w, 3), dtype=np.uint8))
    np.save(labels / "00000000.npy", np.ones((h, w), dtype=np.int32))
    np.savez_compressed(tmp_path / "mapping_outputs.npz", depth=np.ones((1, h, w), dtype=np.float32))
    (tmp_path / "run_manifest.json").write_text(
        json.dumps(
            {
                "frame_paths": ["frames/00000000.png"],
    ...
    seg_quadrant = _read_first_frame(tmp_path / "videos" / "qc_render.mp4")[:h, w:]
    assert seg_quadrant.max() > 0, "segmentation quadrant rendered black from a .npy label cache"


def test_render_offline_video_raises_when_recorded_labels_are_unreadable(tmp_path) -> None:
    h, w = 16, 24
    frames = tmp_path / "frames"
    frames.mkdir()
...
def test_present_class_ids_filters_to_classes_visible_under_mask() -> None:
...
def test_build_legend_text_column_fits_longest_label() -> None:
