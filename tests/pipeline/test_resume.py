...
def _write_prepared_frame(output_dir: Path, idx: int, h: int = 4, w: int = 6) -> None:
    ...
    cv2.imwrite(str(output_dir / "labels" / f"{stem}.png"), np.full((h, w), idx, dtype=np.uint8))
...
def _write_legacy_npy_labels(output_dir: Path, idx: int, h: int = 4, w: int = 6) -> Path:
    """Write a pre-v2 label cache: int32 `.npy`, as v1.0.0's orchestrator saved it."""
    stem = f"{idx:08d}"
    (output_dir / "labels").mkdir(parents=True, exist_ok=True)
    path = output_dir / "labels" / f"{stem}.npy"
    np.save(path, np.full((h, w), idx, dtype=np.int32))
    return path


def test_load_prepared_frames_reads_legacy_npy_labels(tmp_path: Path) -> None:
    """Runs archived before the PNG migration must still load via `view-run`."""
    for idx in (0, 1, 2):
        _write_prepared_frame(tmp_path, idx)
        (tmp_path / "labels" / f"{idx:08d}.png").unlink()
        _write_legacy_npy_labels(tmp_path, idx)

    sidecar = {"key": "k", "frame_indices": [0, 1, 2], "clip_counts": [3]}
    fb = resume_mod.load_prepared_frames(tmp_path, sidecar, np.eye(3, dtype=np.float64))

    assert fb is not None
    assert all(int(f.labels.max()) == int(f.frame_index) for f in fb.frames)
    # Narrowed to uint8 so the .npy path is indistinguishable from the PNG one,
    # and so cv2.resize never sees a CV_32S label map.
    assert all(f.labels.dtype == np.uint8 for f in fb.frames)
    assert all(f.labels_path is not None and f.labels_path.suffix == ".npy" for f in fb.frames)


def test_load_prepared_frames_prefers_png_over_legacy_npy(tmp_path: Path) -> None:
    _write_prepared_frame(tmp_path, 0)
    np.save(tmp_path / "labels" / "00000000.npy", np.full((4, 6), 99, dtype=np.int32))

    sidecar = {"key": "k", "frame_indices": [0], "clip_counts": [1]}
    fb = resume_mod.load_prepared_frames(tmp_path, sidecar, np.eye(3, dtype=np.float64))

    assert fb is not None
    assert fb.frames[0].labels_path is not None
    assert fb.frames[0].labels_path.suffix == ".png"
    assert int(fb.frames[0].labels.max()) == 0


def test_read_labels_file_rejects_ids_outside_uint8(tmp_path: Path) -> None:
    path = tmp_path / "00000000.npy"
    np.save(path, np.full((2, 2), 300, dtype=np.int32))

    with pytest.raises(ValueError, match="outside 0-255"):
        resume_mod.read_labels_file(path)
