...
def test_loger_disables_per_frame_proxy_path() -> None:
...
def test_loger_target_resolution_uses_patch_multiple() -> None:
    assert _nearest_multiple(448, 14) == 448
    assert _nearest_multiple(450, 14) == 448
    assert _nearest_multiple(6, 14) == 14


def test_scale_intrinsics_matches_resized_frame() -> None:
...
def test_assert_pose_convention_accepts_canonical_sequence() -> None:
    poses = np.stack(
        [
...
def test_assert_pose_convention_rejects_non_identity_first_pose() -> None:
    poses = np.stack(
        [
...
def test_assert_pose_convention_rejects_reflected_rotation() -> None:
...
def test_reanchor_makes_first_pose_identity_and_preserves_relative_motion() -> None:
...
def test_reanchor_transforms_world_points_into_camera_zero_frame() -> None:
...
def test_estimate_intrinsics_from_local_points_recovers_focal() -> None:
    h, w = 6, 8
    fx, fy = 120.0, 140.0
    cx, cy = 3.0, 2.0
...
def test_loger_refine_intrinsics_returns_none_without_local_points() -> None:
    backend = LoGeRBackend.__new__(LoGeRBackend)
