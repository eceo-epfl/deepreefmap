...
resolution by default; `--processing-width/height` (and the GUI spinboxes) remain
the user override. Camera-native is the fallback when segmentation is skipped or
...
def test_default_is_model_native_when_segmenting():
...
def test_default_falls_back_to_profile_when_skipping_segmentation():
    assert _default_processing_size("coralscapes-vit-s-dpt", True, _PROFILE) == _PROFILE


def test_default_falls_back_to_profile_for_unknown_model():
    assert _default_processing_size("mystery-model", False, _PROFILE) == _PROFILE


def test_explicit_override_wins_over_native_default():
...
def test_one_sided_override_is_rejected():
