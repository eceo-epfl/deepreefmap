...
from deepreefmap.gui.live_frame_cloud import LiveFrameCloudCache
...
class ViserLiveApp:
    ...
    def _show_geometry_preview(self, geometry_xyz: np.ndarray, geometry_rgb: np.ndarray) -> None:
        """Render the orchestrator's mid-run geometry preview at /preview/cloud."""
    ...
    def set_data(
        ...
        **_ignored: object,
        ...
        The orchestrator also emits a mid-run geometry preview
        (``geometry_xyz``/``geometry_rgb`` with no ``reference_cloud``); render it as a
        throwaway cloud that the full semantic load replaces.
        """
