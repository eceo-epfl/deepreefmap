...
def _save_classes(root, classes_config: "ClassConfig") -> None:
    ...
    groups_int = np.array([c.group_intermediate for c in classes_config.classes], dtype=object)
    groups_coarse = np.array([c.group_coarse for c in classes_config.classes], dtype=object)
...
def _load_classes(root) -> "ClassConfig":
    ...
    for i in range(len(ids)):
        ...
        classes.append(SemanticClass(
            ...
            group_intermediate=str(groups_int[i]),
            group_coarse=str(groups_coarse[i]),
        ))
...
class SceneFrameAccessor:
    ...
    touches exactly one compressed block per dataset.
    """
...
class RunDirFrameAccessor:
    ...
    Mirrors ``SceneFrameAccessor`` so a ``LazyFrameBatch`` can back the viewer with
    the run folder itself, not just a ``.drm`` scene file. The PNG caches are
    lossless, so a reloaded array equals the one preprocessing held in RAM.
    """
...
def lazy_frame_batch_from_run_dir(run_dir: Path, frame_batch: FrameBatch) -> LazyFrameBatch:
    ...
    Only the viewer handoff swaps to this, so the ~9 GiB decoded frame stack is
    freed when the run scope exits instead of staying pinned for the window's life.
    """
